...
public class OfficeUtils {
    ...
    public static String toUrl(File file) {
        ...
        String rePath = url.endsWith("/") ? url.substring(0, url.length() - 1) : url;
        System.out.println("处理的文档路径为：" + rePath);
        return rePath;
    }

    public static File getDefaultOfficeHome() {
    	String programFiles = PropertiesReader.getInstance().getProperty("openOfficePath");
    	if(StringUtils.isNotBlank(programFiles)){
    		return getOfficeExecutable(new File(programFiles));
    	}else{
    ...
    public static File getOfficeExecutable(File officeHome) {
        if (PlatformUtils.isMac()) {
            ...
            return new File(officeHome, "/soffice.bin");
        }
    }
